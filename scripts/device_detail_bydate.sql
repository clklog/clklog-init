INSERT INTO ${CLKLOG_LOG_DB}.device_detail_bydate
...
FROM (
         SELECT lib, project_name, is_first_day, country, province
              , manufacturer, count(t1.pv) AS pv, COUNTDistinct(t1.user) AS uv
         ...
         LEFT JOIN (
    SELECT lib, project_name, is_first_day, country, province
         , manufacturer, count(1) AS visitCount, sum(diff) AS visitTime
