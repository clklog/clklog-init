INSERT INTO ${CLKLOG_LOG_DB}.flow_trend_byhour
...
FROM (
         SELECT stat_hour, lib, project_name, is_first_day, country
              , province, count(t1.pv) AS pv, COUNTDistinct(t1.user) AS uv
         ...
         LEFT JOIN (
    SELECT stat_hour, lib, project_name, is_first_day, country
         , province, count(1) AS visitCount, sum(diff) AS visitTime
    ...
    WHERE indexOf(stat_dates, toDate(':cal_date')) = 1
      AND indexOf(stat_hours, stat_hour) = 1
    GROUP BY stat_hour, lib, project_name, is_first_day, country, province WITH CUBE
    ) t4
                   ON t2.lib = t4.lib
