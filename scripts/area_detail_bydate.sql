INSERT INTO clklog.area_detail_bydate
SELECT ':cal_date' AS stat_date
     ...
     , multiIf(t2.country = '', 'all', t2.country = 'N/A', 'δ֪����', t2.country) AS country
     , multiIf(t2.province = '', 'all', t2.province = 'N/A', 'δ֪ʡ��', t2.province) AS province
     , multiIf(t2.city = '', 'all', t2.city = 'N/A', 'δ֪����', t2.city) AS city
...
FROM (
         SELECT lib, project_name, is_first_day, country, province
              , city, count(t1.pv) AS pv, COUNTDistinct(t1.user) AS uv
         ...
         LEFT JOIN (
    SELECT lib, project_name, is_first_day, country, province
         , city, count(1) AS visitCount, sum(diff) AS visitTime
