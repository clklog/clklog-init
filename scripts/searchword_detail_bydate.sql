...
FROM (
         SELECT lib, project_name, is_first_day, country, province
              , latest_search_keyword, count(t1.pv) AS pv, COUNTDistinct(t1.user) AS uv
         ...
         FROM (
                  ...
                  WHERE stat_date = ':cal_date'
                    AND latest_search_keyword NOT IN ('ȡֵ�쳣', 'δȡ��ֵ_ֱ�Ӵ�', 'δȡ��ֵ', 'url��domain����ʧ��', '')
                  ) t1
         GROUP BY lib, project_name, is_first_day, country, province, latest_search_keyword WITH CUBE
         ) t2
         LEFT JOIN (
    SELECT lib, project_name, is_first_day, country, province
         , latest_search_keyword, count(1) AS visitCount, sum(diff) AS visitTime
    ...
    GROUP BY lib, project_name, is_first_day, country, province, latest_search_keyword WITH CUBE
                   ...
                   ON t2.lib = t4.lib
                       ...
                       AND t2.latest_search_keyword = t4.latest_search_keyword