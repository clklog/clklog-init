...
SELECT ':cal_date' AS stat_date
     ...
     , multiIf(t2.latest_referrer_host = '', 'all', t2.latest_referrer_host = 'N/A', 'ֱ�ӷ���', t2.latest_referrer_host) AS latest_referrer_host
...
FROM (
         SELECT lib, project_name, is_first_day, country, province
              , latest_referrer_host, count(t1.pv) AS pv, COUNTDistinct(t1.user) AS uv
         ...
         FROM (
                  SELECT lib, project_name, is_first_day
                       ...
                       , if(latest_referrer_host = ''
                                OR latest_referrer_host = 'url��domain����ʧ��', 'N/A', latest_referrer_host) AS latest_referrer_host
                       , multiIf(lib = 'js'
                  ...
                  WHERE stat_date = ':cal_date'
                    AND latest_referrer_host NOT IN ('ȡֵ�쳣')
                  ) t1
         GROUP BY lib, project_name, is_first_day, country, province, latest_referrer_host WITH CUBE
         ) t2
         LEFT JOIN (
    SELECT lib, project_name, is_first_day, country, province
         , latest_referrer_host, count(1) AS visitCount, sum(diff) AS visitTime
    ...
    GROUP BY lib, project_name, is_first_day, country, province, latest_referrer_host WITH CUBE
                   ...
                   ON t2.lib = t4.lib
                       ...
                       AND t2.latest_referrer_host = t4.latest_referrer_host